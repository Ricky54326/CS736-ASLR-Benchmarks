jpeg("entropy1-freebsd.jpg");
vec <- read.csv("/home/r/riccardo/Development/CS736-ASLR-Benchmarks/entropy1-freebsd.txt-clean.csv")
#vec <- read.csv("http://www.upl.cs.wisc.edu/~riccardo/entropy1-debian-clean.csv")
y <- vec$high
x <- vec$low


plot(x, y, main="FreeBSD - Stack", xaxt = "n", yaxt = "n",
     xlab="Low 8 bits ", ylab="Higher 8 bits ", pch=19, cex=0.1)
